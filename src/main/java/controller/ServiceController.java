...
public class ServiceController extends HttpServlet {
    ...
    private static final String[] ALLOWED_STATUSES = {"Ingresado", "Asignado", "En Progreso", "Resuelto"};
   ...
   @Override
    ...
    try {
        ...
        if (reference == null || reference.isBlank()) {
            writeError(resp, HttpServletResponse.SC_BAD_REQUEST, "'reference' es obligatorio.");
            return;
        }
        ...
        if (serviceData.findById(serviceId) == null) {
            writeError(resp, HttpServletResponse.SC_BAD_REQUEST,
                    "serviceId inválido: no existe en la base de soporte.");
            return;
        }
        ...
        if (issueTimestamp == null) {
            issueTimestamp = LocalDateTime.now();
        }
        ...
        mapper.writeValue(resp.getWriter(), Map.of(
                "message", "Issue registrado en soporte.",
                "reference", reference,
                "serviceId", serviceId,
        ...
        writeError(resp, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
                "No se pudo guardar la solicitud en soporte: " + e.getMessage());
    }
}
